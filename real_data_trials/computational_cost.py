...
for i in range(MIN_NODES, MAX_NODES):
    ...
    for j in range(NUM_TRIALS):
        ...
        model = GraphQ(input_dim=10, hidden_dim=16, output_dim=1, gamma=0.8)
        device = (torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu'))
        model.to(device)
        model.train(env, num_episodes=20)

        total_time_for_eopch_gnn += model.get_train_gnn_agent_time()
    
    total_time_for_epoch /= NUM_TRIALS
    total_time_for_epoch_dqn /= NUM_TRIALS
    total_time_for_eopch_gnn /= NUM_TRIALS
    times.append(total_time_for_epoch)
    times_dqn.append(total_time_for_epoch_dqn)
    times_gnn.append(total_time_for_eopch_gnn)
    ...
    print(f"gnn average training time for graph with {i} nodes: {total_time_for_eopch_gnn}")
