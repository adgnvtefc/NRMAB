...
def visualize_trained_agent(G, model, num_actions):
    # Build the reverse mapping after the graph has been created
    ...
    while True:
        ...
        seeded_nodes = select_action_double_dqn(G, model, num_actions)

        # Apply actions and state transitions
        ns.passive_state_transition_without_neighbors(G, exempt_nodes=seeded_nodes)
        ns.active_state_transition(seeded_nodes)
...
#     num_nodes = 50
#     num_edges = 50
#     num_actions = 4  # Number of nodes to activate per timestep
...
#     model, policy = train_double_dqn_agent(config, num_actions)

#     print("Training completed. Starting visualization...")
#     visualize_trained_agent(G, model, num_actions)
