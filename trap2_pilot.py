...
Results -> real_data_trials/results_trap2/history/.
"""
...
HARD = {"aa": (0.85, 0.95), "ap": (0.0, 0.05), "pa": (0.85, 0.95), "pp": (0.0, 0.0)}
NUM_ACTIONS = 10
NUM_COMPARISONS = 50
CASCADE_PROB = 0.8
GAMMA = 0.8
...
def pretrain_distill(graph, n_states=200, epochs=60, batch=8, lr=1e-3):
    N = len(graph.nodes())
    model = CDSQN(N, 10, 64, 64, nodewise=True, dueling=False, linear_head=True).to(device)
...
for seed_i in range(NUM_SEEDS):
    ...
    model = CDSQN(len(graph.nodes), 10, 64, 64, nodewise=True, dueling=False, linear_head=True).to(device)
    ...
    model, _ = train_cdsqn_agent(cfg, NUM_ACTIONS, learning_rate=3e-4, model=model, **TRAIN)
    ...
    plot_trials(results, output_dir="real_data_trials/results_trap2",
